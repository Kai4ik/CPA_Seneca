...
bool LineManager::runt(std::ostream& os)
{
	static int iter = 0;
	os << "Line Manager Iteration: " << ++iter << std::endl;
	...
	CustomerOrder temp;
	while (!AssemblyLine.front()->getIfCompleted(temp))
	{
		std::for_each(this->AssemblyLine.begin(), this->AssemblyLine.end(), [&](Workstation* obj)
			{
				obj->runProcess(os);
			});
		std::for_each(this->AssemblyLine.begin(), this->AssemblyLine.end(), [&](Workstation* obj)
		...
		std::for_each(this->AssemblyLine.begin(), this->AssemblyLine.end(), [&](Workstation* obj)
			{
				if (obj->getIfCompleted(temp1)) Completed.push_front(std::move(temp1));
			});
	}
}
