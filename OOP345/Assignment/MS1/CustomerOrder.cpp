...
CustomerOrder::CustomerOrder(std::string& record)
{
	...
	m_lstItem = new Item*[this->m_cntItem];
...
//this functions checks item fill_state and returns it
...
//this function check if all items  in the order have been filled or no
bool CustomerOrder::isOrderFilled() const
{
	unsigned filled_items = 0;
	for (unsigned i = 0; i < this->m_cntItem; ++i)
		this->m_lstItem[i]->m_isFilled == true ? filled_items++ : filled_items + 0;
	return (filled_items == this->m_cntItem ? true : false);
}

//function that fills all items in order which match to the given station name(station passed as a parameter), also it changes quantity of that item in the station
